        ...
        if not args.skip_crawl:
            if await wap.have_attacks_started() and not args.resume_crawl:
                ...
                if hasattr(signal, 'SIGINT'):
                    try:
                        original_handler = signal.signal(signal.SIGINT, inner_ctrl_c_signal_handler)
                ...
                try:
                    ...
                    if original_handler is not None and hasattr(signal, 'SIGINT') and hasattr(signal, 'SIG_DFL'):
                        try:
