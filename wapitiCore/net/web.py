...
def make_absolute(base: str, url: str, allow_fragments=True) -> str:
    ...
    url_path = os.path.normpath(url_path.replace("\\", "/"))
    ...
    if parts.scheme:
        ...
        new_path = os.path.normpath(current_directory + url_path)
...
class Request:
    ...
    def dir_name(self) -> str:
        if self.file_name:
            return os.path.dirname(self._resource_path) + "/"
        return self._resource_path
    ...
    def parent_dir(self) -> str:
        ...
        if self.is_root:
            return self._resource_path
        return os.path.dirname(os.path.dirname(self._resource_path)) + "/"
...
def http_repr(request: Request, left_margin: str = "    ") -> str:
    # Fix IndexError when splitting URL - handle case where URL doesn't have enough parts
    url_parts = request.url.split('/', 3)
    if len(url_parts) >= 4:
        rel_url = url_parts[3]
    else:
        # Handle edge case where URL might not have enough parts after splitting
        # This could happen with URLs like "http://example.com" without a path
        rel_url = ""
        if len(url_parts) >= 3:
            rel_url = url_parts[2]
