...
class ModuleSsl(Attack):
    ...
    async def process_sslscan(self, hostname: str, port: int) -> AsyncIterator[Tuple[int, str]]:
        with tempfile.NamedTemporaryFile("r", suffix=".xml", delete=False) as temp_file:
            ...
            sslscan_executable = "sslscan.exe" if system == "Windows" else "sslscan"
            sslscan_command = [sslscan_executable, "--iana-names", "--ocsp", f"--xml={temp_file.name}", f"{hostname}:{port}"]
            try:
                # Use shell=True on Windows to handle .exe files properly
                subprocess.run(sslscan_command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True, shell=(system == "Windows"))
