    ...
    async def attack(self) -> bool:
        ...
        async with AsyncCrawler.with_configuration(self.crawler_configuration) as crawler:
            ...
            for attack_module in attack_modules:
                ...
                # Setup signal handler to prompt the user for task cancellation only if we're in the main thread
                import threading
                is_main_thread = threading.current_thread().name == "MainThread"
                if is_main_thread and hasattr(signal, 'SIGINT'):
                    try:
                        signal.signal(signal.SIGINT, self.handle_user_interruption)
                    except (ValueError, OSError):
                        logging.warning("Signal handling not available on this platform")
                ...
                try:
                    ...
                    # Clean up the signal handler for the next loop only if we're in the main thread
                    if is_main_thread and hasattr(signal, 'SIGINT') and hasattr(signal, 'SIG_DFL'):
                        try:
                            signal.signal(signal.SIGINT, signal.SIG_DFL)
                        except (ValueError, OSError):
                            logging.warning("Could not restore signal handler")
